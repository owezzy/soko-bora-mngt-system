...
export class QuickChatComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    selectChat(id: string | undefined): void
    {
        ...
        // Get the chat data
        this._quickChatService.getChatById(<string>id).subscribe();
    }
    ...
    evaluateMessageDate(i: number, chat: any, message: any):boolean {
     // return   (chat.messages[i - 1].createdAt | date:'d') !== (message.createdAt | date:'d')
     const date_1 = chat?.messages[i - 1].createdAt
        const initialDate =  this._datePine.transform(date_1, 'd')
        const createdDate =  this._datePine.transform(message.CreatedAt, 'd')
